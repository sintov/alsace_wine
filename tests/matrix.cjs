...
(async()=>{
 ...
 const gotoMatrix=async()=>{await page.locator('[data-tab=matrix]').click();await page.locator('#mx-evidence').selectOption('all')};
 ...
 assert.ok((await label('Riesling','marl-limestone')).includes(': tasted')===false);
 ...
 await page.locator('#mx-area').selectOption('south');assert.ok((await label('Riesling','granite','south')).includes(': tasted')===false,'tasting in colmar does not cover south');
 ...
 assert.ok((await label('Riesling','alluvial')).includes(': tasted')===false,'blend must not mark Riesling');assert.equal(await cell('Riesling','alluvial').locator('.mx-blend').count(),1,'blend hint shown');
 ...
 assert.ok((await label('Riesling','mixed-other')).includes('tasted: 1'));assert.ok((await label('Riesling','sandstone')).includes(': tasted')===false,'mixed site is not pure sandstone');assert.ok((await label('Riesling','granite')).includes('tasted: 2 encounters'),'mixed site is not pure granite');
 ...
 await addWine({grape:'Pinot Gris',area:'colmar',cuvee:'AreaOnly'});assert.ok((await label('Pinot Gris','')).includes('tasted: 1'),'area-only counts under unknown geology');assert.ok((await label('Pinot Gris','marl-limestone')).includes(': tasted')===false);assert.deepEqual(await combos(),{single:2,blend:1},'no invented geology');
 await page.locator('#mx-area').selectOption('south');assert.ok((await label('Pinot Gris','','south')).includes(': tasted')===false);await page.locator('#mx-area').selectOption('all');
 ...
 await gotoMatrix();assert.ok((await label('Riesling','mixed-other')).includes(': tasted')===false);assert.ok((await label('Riesling','granite')).includes('tasted: 3 encounters'));assert.deepEqual(await combos(),{single:1,blend:1});
 ...
 await page.locator('#mx-mode').selectOption('area-geo');await page.locator('#mx-grape').selectOption('Riesling');assert.ok((await label('Riesling','granite','colmar')).includes('tasted: 1'));assert.ok((await label('Riesling','granite','south')).includes(': tasted')===false);assert.equal(await page.locator('.mx tbody tr').count(),7);
